...
const CalculateCGPAScreen = ({ navigation }) => {
    ...
    const calculate = () => {
        ...
        navigation.navigate('Result', { cgpa, subjects });
    };
    ...
    const Save = () => {
        if (!name) {
            ...
            } else if (creditHours < 0 || creditHours > 4) {
                alert('Please Enter Credit Hours between 0-4')
            } else {
                const subjectGpa = getSubjectGpa();
                const values = { marks, creditHours, subjectGpa, name };
...
export default CalculateCGPAScreen;
